...
class ExcelParser():
    ...
    def writeCellValue(self,rowNo,colNo,value,color=None):
        ...
        if color:
            if color=="red":
                self.sheet.cell(row=rowNo, column=colNo).font = Font(name="黑体",bold=False, size=10,color=colors.RED)
            elif color=="green":
                self.sheet.cell(row=rowNo, column=colNo).font = Font(name="黑体",bold=False, size=10,color=colors.GREEN)
        else:
            self.sheet.cell(row=rowNo, column=colNo).font = Font(name="黑体",bold=False, size=10,color=colors.BLACK)
        side= Side(style='thin', color="000000")
...
if __name__=="__main__":
    ...
    excelObj= ExcelParser(r"E:\我的坚果云\framework\practice\app_ui_auto\TestData\邢帅教育.xlsx")
    ...
    sheet=excelObj.getSheetByName("ssstttt")
    ...
    excelObj.insertImage("d:\\pydelete\\0721\\1.png",(242,130),"B3")
    excelObj.saveExcel()
